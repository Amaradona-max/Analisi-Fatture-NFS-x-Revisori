...
async def process_file(background_tasks: BackgroundTasks, file: UploadFile = File(...)):
    ...
    try:
        ...
        background_tasks.add_task(_run_single_file_task, task_id, NFSFTFileProcessor(), upload_path, output_path)
...
async def process_file_pisa(background_tasks: BackgroundTasks, file: UploadFile = File(...)):
    file_ext = Path(file.filename).suffix.lower()
    if file_ext not in settings.ALLOWED_EXTENSIONS:
        raise HTTPException(
            status_code=400,
    ...
    try:
        ...
        tasks[task_id] = {"status": "queued", "file_id": task_id}
        background_tasks.add_task(_run_single_file_task, task_id, PisaFTFileProcessor(), upload_path, output_path)
...
async def process_compare(background_tasks: BackgroundTasks, nfs_file: UploadFile = File(...), pisa_file: UploadFile = File(...)):
